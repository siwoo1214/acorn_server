...
public class Login  extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result) {
			...
			//1) 세션저장소 얻어오기 => 키를 받아오고 그 키가 나를 인증하는 정보
			//              => 사용자에 대한 저장소 얻어오기(세션ID값으로 얻어온다)
			//세션id => 서버에 최초에 접속하면 서버가 발급하는 키
			
			HttpSession session  = request.getSession();  //여기서 얻어왔어 세션을  
			//저장하기 (서버에서 설정한 세션의 유지기간이 끝날동안에)
			session.setAttribute("id", id);  //세션 설정하기
